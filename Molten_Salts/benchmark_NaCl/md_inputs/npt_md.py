...
RUN_METADATA_SCHEMA = "extended_S0.molten_npt_benchmark_run.v2"
...
def frame_count(path: Path) -> int:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "or delete anything in this output directory."
        ) from exc
    finally:
...
def build_run_metadata(
    ...
    direct_replica = bool(args.direct_replica)
    return {
        ...
        "nvt_steps": 0 if direct_replica else int(args.nvt_steps),
        "protocol": (
            "direct_isotropic_mtk_npt"
            if direct_replica
            else "fire_nose_hoover_chain_nvt_then_isotropic_mtk_npt"
        ),
        "npt_steps": int(args.npt_steps),
        "interval": int(args.interval),
        "direct_replica": direct_replica,
        "optimization_fmax": None if direct_replica else float(args.fmax),
