"""Charge-neutral LiCl-NaCl thermodynamics from S(0), for two k^2 cuts.

Same machinery as compare_mgcl2_nacl_kcut_workflow_3x1_newdata.py, retargeted at
LiCl-NaCl.  Two things genuinely differ, beyond renaming:

1. The stoichiometry.  LiCl-NaCl is charge-symmetric, so mixing (1-y) NaCl with
   y LiCl gives n_Li = y, n_Na = 1-y, n_Cl = 1, two ions per formula unit:

       x_Li = y/2,   x_Na = (1-y)/2,   x_Cl = 1/2  (constant!)
       dx_Li/dy = 1/2                              (constant!)

   For MgCl2-NaCl the anion count grows with y, giving x_Mg = y/(2+y) and
   dx_Mg/dy = 2/(2+y)^2.  Using the MgCl2 transform here would be wrong.

   The chemical potentials are mu_LiCl = mu_Li + mu_Cl and
   mu_NaCl = mu_Na + mu_Cl, i.e. one anion each rather than two and one.

2. There is no experimental comparison.  nist_experimental_thermodynamics holds
   a NIST assessment for MgCl2-NaCl only; no equivalent LiCl-NaCl numbers are
   available here, so panel (c) shows the GP curves and the ideal curve and
   nothing else.  Plotting the MgCl2 coefficients on a LiCl axis would invent a
   comparison that does not exist.

Everything else -- the bare-Coulomb S(0) input, the GP over the excess-free-energy
curvature, the boundary-respecting integration, the block error bars -- is
unchanged.
"""
...
# nist_experimental_thermodynamics is deliberately NOT imported: its coefficients
# are a NIST assessment of MgCl2-NaCl and say nothing about LiCl-NaCl.


HERE = Path(__file__).resolve().parent
SOURCE_ROOT = Path("/Users/xiaoyuwang/Project/multi_S0_GP_test")
S0_DIR = Path("/Users/xiaoyuwang/Downloads/MoltenSaltData/LiCl_NaCl")
...
def convert_split(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    # (1-y) NaCl + y LiCl -> n_Li = y, n_Na = 1-y, n_Cl = 1, two ions per
    # formula unit.  So x_Li = y/2 and the Jacobian is a constant 1/2, unlike
    # MgCl2-NaCl where the anion count grows with y.
    dx_li_dy = np.full_like(y, 0.5)
    # One anion per formula unit for both components here.
...
def main() -> None:
    ...
    # One or two cuts.  With a single cut only the first style is used.
    _styles = [{"color": "#1764ab", "ls": "-", "marker": "o"},
               {"color": "#e12729", "ls": "--", "marker": "x"}]
    if len(CUTS) == 2:
        _styles = [_styles[1], _styles[0]]      # wider cut dashed/red, as before
    ...
    # Data-driven for the same reason.  Clipped at the dilute ends, where the
    # ideal log term dives to -infinity.
    _b = np.concatenate([
    ...
    # No experimental curves here -- see the note at the top of the file.
    ax3.text(
        0.02, 0.04,
    ...
    # Scale to the data rather than reusing the MgCl2-NaCl limits: this system
    # is far closer to ideal, so its integrand is an order of magnitude smaller.
    # Cap the error bars at three times their median before letting them set the
    # limits.  With only four blocks a couple of them are enormous and would
    # otherwise leave the actual curve as a flat line near the top.
