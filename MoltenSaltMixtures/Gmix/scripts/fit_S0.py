...
# Where the S(k) .dat files live.  Overridden by --sk-dir.
SK_ROOT = HERE / "sk_output"

TEMPERATURE_K = 1200.0  # the MD temperature

# Species order used everywhere below, and their charges.
# Species in the order the .dat columns are written, and their charges.
# Overridden by --species / --charges, so one script serves both MgCl2-NaCl
# (Mg,Cl,Na / 2,-1,1) and LiCl-NaCl (Li,Cl,Na / 1,-1,1).  The cation being
# varied is first, the common anion second, the reference cation third.
SPECIES = ("Mg", "Cl", "Na")
CHARGES = np.array([2.0, -1.0, 1.0])

# Physical constants, in eV and Angstrom.
E2_EV_A = 14.399645478425668  # e^2 / (4 pi eps0)
KB_EV_K = 8.617333262e-5  # Boltzmann constant

# The .dat files store k^2 in cycles^2/A^2.  The fit works in angular
# wavenumbers, so k^2 gets multiplied by this before fitting.
CYCLES2_TO_ANGULAR2 = (2.0 * np.pi) ** 2

# For the free-kappa version we run the fit several times from different
# starting guesses and keep the best one, because the fit can otherwise get
# stuck.  These are multiples of the calculated kappa.
START_FACTORS = (1.0, 0.75, 0.5, 0.3)

# How many function evaluations the optimiser is allowed.  Deliberately large:
# a fit that runs out of budget returns wherever it happened to stop, which is
# not an answer at all.  Better to spend the CPU.
MAX_NFEV = 20000

# Which column of the .dat file holds which curve.
# Column layout of the .dat file: k^2, then value/error for each of the six
# pairs in lower-triangle order.  Both dicts are rebuilt by set_species().
...
# A 3x3 symmetric table has 6 independent numbers; these index them.
...
def load_sk(path: Path):
    ...
    # The k = 0 row is special: there the diagonal entries are simply the
    # number of atoms of each species, so we use it to read the composition.
    zero_row = np.isclose(k2, 0.0)
    counts = np.array([raw[zero_row, COLUMNS[f"{s}{s}"]][0] for s in SPECIES])

    # Drop the k = 0 row; it carries no shape information.
    keep = k2 > 0.0
    k2 = k2[keep]
    raw = raw[keep]
    ...
    for name, (i, j) in PAIR_INDEX.items():
        ...
        matrices[:, j, i] = raw[:, COLUMNS[name]]  # the table is symmetric

    # The smallest k the box allows is 1/L, so L follows from the smallest k^2.
...
def fit_one(k2_angular, matrices, kappa_calc, w, free_kappa, kappa_start=None):
    ...
    # Starting guesses.  For A, take the rough estimate and subtract a typical
    # size of the 1/k^2 piece, so that A is what is left once the blow-up is
    # accounted for.  L starts at zero.
    ...
    def residuals(params):
        ...
        try:
            ...
            # Bad parameters: hand back something huge so the optimiser backs off.
            return np.full(target.size + N_TRI + int(free_kappa), 1.0e8)

        misfit = (model[:, TRI[0], TRI[1]] - target) / scale

        # Nudge 1: keep L near zero.  Divided by 1e4, so this only matters if
        # L tries to run away to something absurd.
        nudges = [pack(l - l0) / 10000.0]

        # Nudge 2: keep kappa near the calculated value.  Note this is measured
        # from kappa_calc, never from kappa_start -- that is what keeps the
        # different starts comparable.
        if free_kappa:
            nudges.append(np.array([(params[-1] - log_kappa_calc) / 10.0]))

        # The nudges are part of this list, so the reported rmse averages over
        # them too.  Do not try to reproduce rmse from the data points alone.
        return np.r_[misfit.ravel(), np.concatenate(nudges)]

    # Parameters: 6 numbers for A, 6 for L, and optionally log(kappa).
    # Fitting log(kappa) rather than kappa keeps it positive automatically.
    x0 = np.r_[pack(a0), pack(l0)]
    lo = np.full(len(x0), -np.inf)
    hi = np.full(len(x0), np.inf)
    if free_kappa:
        x0 = np.r_[x0, math.log(start)]
        # Allow kappa within a factor of 10 either side of the calculated value.
    ...
    result = least_squares(
        ...
        x0,
        bounds=(lo, hi),
        # A robust loss, because the very smallest-k points are noisy.
        # Careful: with this loss, a lower "cost" does not always mean a lower
        # rmse.  Judge fits by cost, since that is what is minimised.
        loss="soft_l1",
        f_scale=1.0,
        x_scale="jac",
        max_nfev=MAX_NFEV,
    ...
    return {
        # A and L are returned so a diagnostic plot can redraw the fitted curve
        # at arbitrary k, not just its k = 0 limit.
        "A": a,
        "L": l_matrix,
        # status above 0 means it really converged.  status 0 means it just ran
        # out of evaluations, and the numbers should not be used.
...
def select_start(tries, number_weight, tol=0.20):
    ...
    # Largest basin wins; ties in size fall back to the cheaper member.
...
def run(kcut: float) -> list[dict]:
    ...
    for tag in tags:
        for split, path in split_files(tag).items():
            ...
            # Keep only the small-k points.  Remember kcut is a cut on k^2.
            ...
            for label, free_kappa in (
                ...
                if free_kappa:
                    # Try several starting points.  All of them minimise the
                    # same objective, so their costs are comparable.
                    tries = [
                    ...
                    # kappa is fixed, so there is nothing to vary.
                ...
                rows.append({
                    ...
                    # 1 = a real answer.  0 = ran out of evaluations, ignore it.
                    "fit_converged": int(out["converged"]),
                    "fit_status": out["status"],
                    "fit_nfev": out["nfev"],
                    # Use this, not rmse, to compare two fits.
                    "fit_cost": out["cost"],
                    "fit_rmse": out["rmse"],
                    "n_starts": len(START_FACTORS) if free_kappa else 1,
                    "n_starts_converged": n_good,
                    "best_start_factor": factor,
                    # How the starts were reconciled.  "modal-basin" means the
                    # starts split into groups with very different S(0) and the
                    # most-populated group was taken; look at alt_* to see what
                    # was set aside, and treat such rows as uncertain.
                    "n_basins": n_basins,
                    "selection_rule": rule,
                    "alt_S0_NN": alt[0] if alt else float("nan"),
                    "alt_cost": alt[1] if alt else float("nan"),
                    "kappa_calculated": kappa_calc,
                    # Equals kappa_calculated in the fixed version.  In the free
                    # version, do not read this as a screening length.
                    "kappa_used": out["kappa"],
                    **{f"S0_{lab}": s0[i, j]
                       for lab, (i, j) in PAIR_INDEX.items()},
                    "S0_NN": float(nw @ s0 @ nw),
                    # Always zero on purpose.  Sanity check only.
    ...
    for tag in tags:
        for label in ("BC fixed kappa_D", "BC fitted kappa_D"):
            ...
            for col in cols:
                ...
                row[col] = whole[col]  # the value: whole-window fit
                row[col + "_err"] = (
