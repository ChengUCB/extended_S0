...
Run one system:
    ...
    python3 compute_sk.py --root "/Volumes/Extreme Pro/SelectedMgCl2-NaCl_MD" \
...
# The species, in the order the six curves are written.  --species overrides it,
# so the same script serves MgCl2-NaCl and LiCl-NaCl (and anything else with
# three species).  The cation being varied goes first, the common anion second.
SPECIES = ("Mg", "Cl", "Na")
...
# Defaults chosen to match how the existing sk_output files were made.
K_BINS = 8            # h,k,l each run 0..7  ->  512 k points
FRAME_PS = 0.4        # spacing between saved frames, from md.log
START_PS = 50.0       # H.txt says the analysis window was 50-250 ps
END_PS = 250.0
N_ERROR_BLOCKS = 5    # sub-blocks used only for the error column
...
def process_one(tag: str, root: Path, out_dir: Path, source: str,
    ...
    # --last-ps takes the final N ps of whichever trajectory this is, so the
    # window length is fixed even though the runs are of different lengths.
    if last_ps:
    ...
    # end_ps <= 0 means "run to the end of the trajectory", which is useful
    # because the trajectories are not all the same length.
    to_the_end = end_ps <= 0.0
    last = 1 << 30 if to_the_end else int(round(end_ps / frame_ps))
    # Either fixed-length blocks (--block-ps) or a fixed number of equal pieces
    # (--n-blocks).  The second matches the original sk_proc_traj.py, which cut
    # the window into n_splits pieces regardless of how long they came out.
