#!/usr/bin/env python3
"""Extract split-0 S0 values with bare-Coulomb matrix S-loss fits."""
...
def charge_projection_matrix(fractions: np.ndarray) -> np.ndarray:
    ...
    w = np.sqrt(np.asarray(fractions, dtype=float)) * z
...
def process_sk_file(payload: tuple[common.SkFile, tuple[float, ...], np.ndarray, np.ndarray, bool]) -> list[dict[str, object]]:
    ...
    for kcut in kcuts:
        ...
        s0_ox, n_direct, rmse_direct, ok_direct = dh_common.fit_matrix_oz_s_loss(
            k2,
            ox_matrix,
            direct_mask,
            max_nfev=2000,
        ...
        for method, result in results:
            ...
            common.set_direct_ox_fields(row, s0_ox)
            direct_oo = row["S0_OO"]
            common.set_allpair_fields(row, item, result.s0)
            row["S0_OO"] = direct_oo
            row["kappa_D_input"] = kappa_input
            row["kappa_D_fit"] = result.kappa_fit
            row["n_fit_points"] = max(n_direct, result.n_fit)
            row["fit_rmse_scaled"] = float(np.nanmean([rmse_direct, result.rmse]))
            row["fit_success"] = int(ok_direct and result.success)
            rows.append(row)
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=SCRIPT_DIR / "S0_NaClWater_matrix_BC_s_loss_split0.csv")
