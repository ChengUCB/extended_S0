#!/usr/bin/env python3
"""Extract split-0 S0 values with matrix DH S-loss fits."""
...
def process_sk_file(payload: tuple[common.SkFile, tuple[float, ...], np.ndarray, np.ndarray]) -> list[dict[str, object]]:
    ...
    for kcut in kcuts:
        ...
        s0_ox, n_direct, rmse_direct, ok_direct = fit_matrix_oz_s_loss(k2, ox_matrix, direct_mask, max_nfev=2000)
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=SCRIPT_DIR / "S0_NaClWater_matrix_DH_s_loss_split0.csv")
