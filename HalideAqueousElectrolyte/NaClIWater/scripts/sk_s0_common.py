"""Common file I/O, preprocessing, and CSV helpers for NaClIWater split-0 S0 extraction."""
...
DAT_RE = re.compile(
    r"^O(?P<o>\d+)-Na(?P<na>\d+)-Cl(?P<cl>\d+)-I(?P<i>\d+)(?:-(?P<split>\d+))?-allSk\.dat$"
)

COMPONENTS = ("O", "Na", "Cl", "I")
ALLPAIR_COMPONENTS = ("OO", "ONa", "OCl", "OI", "NaNa", "NaCl", "NaI", "ClCl", "ClI", "II")
ALLPAIR_MAP = {
    "OO": (0, 0),
    "ONa": (0, 1),
    "OCl": (0, 2),
...
class SkFile:
    ...
    @property
    def n_x(self) -> int:
        return self.n_na + self.n_cl + self.n_i
    ...
    def fractions(self) -> np.ndarray:
        return np.array([self.n_o, self.n_na, self.n_cl, self.n_i], dtype=float) / self.n_total
    ...
    @property
    def x_i_anion(self) -> float:
        denominator = self.n_cl + self.n_i
        return self.n_i / denominator if denominator else math.nan
...
def discover_split_files(data_dir: Path, source: str = "main", split: int = 0) -> list[SkFile]:
    ...
    for path in sorted(data_dir.glob("O*-Na*-Cl*-I*-allSk.dat")):
        item = parse_sk_file(path, source)
        if item is None or item.split != split:
            continue
        key = (item.n_o, item.n_na, item.n_cl, item.n_i)
        if key in seen:
    ...
    return sorted(files, key=lambda item: (item.x_x, item.x_i_anion))
